...
from sample_utuils import get_all_v_colors, uniformSample
from global_shape_condition import get_shape_condition
from sample_utuils import create_sample_tex_img
...
# def save_uniform(uniform_num=2048, pixel_num=128):
#     tex_list = []
#     row_tex_list = []
...
#             uniform_path = os.path.join(gt_dir, name, f'uniform_{uniform_num}.npz')
#             uniform_init_data = np.load(uniform_path)
#             sample_colors = uniform_init_data['colors']
#             sample_coords = uniform_init_data['texs']

#             tex = create_sample_tex_img(torch.from_numpy(sample_coords), torch.from_numpy(sample_colors), torch.from_numpy(mask), pixel_num, r=3, use_mask=False)
#             tex[:, :, [0, 2]] = tex[:, :, [2, 0]]
#             tex_list.append(tex)
...
#     plt.imsave(f'{root}/output/2048/uniform_tex_nomask.png', tex)
...
with open(train_split_file, 'r') as f:
    ...
    for line in f:
        if index > -1:
            ...
            # uv_img = cv2.imread(uv_path) / 255.
            # save_uniform(model_path, file_path, 10000, uv_img)
