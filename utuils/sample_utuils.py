...
def get_all_v_colors(tex_coords, uv_img):
    ...
    for tex_coord in tex_coords:
        ...
        v = int((1 - tex_coord[1]) * (uv_img.shape[0] - 1))
...
def create_sample_tex_img(coords, colors, mask, pixel_num, r=3, use_mask=True):
    if use_mask:
        img = torch.full((pixel_num, pixel_num, 3), 1).float() * mask.cpu()
    else:
        img = torch.full((pixel_num, pixel_num, 3), 1).float()
...
def get_all_colors(uv_coords, uv_imgs):
    ...
    for coords, uv_img in zip(uv_coords, uv_imgs):
        ...
        for coord in coords:
            ...
            v = int((1 - coord[1]) * (uv_img.shape[0] - 1))
