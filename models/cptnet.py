...
class CPTNet(nn.Module):
    def __init__(self, args):
        ...
        # self.color_vae = ColorVAE(input_dim=3, embedding_dim=self.COLOR_EMB_DIM, hidden_dim=64, pixel_num=self.PIXEL_NUM)
    ...
    def forward(self, color_embeddings, shape_conditions, data):
        uv_imgs, coord_imgs, normal_imgs, masks, sample_points = data
        ...
        # print("Position Emb", position_embeddings.shape)
        
        # ======DIFF======
        # print("Color Emb", color_embeddings.shape)
        # print("Shape Emb", shape_conditions.shape)
        ...
        # print("ColorRange", pred_colors.min().item(), pred_colors.max().item())
        # to uv
        feature_input = pred_colors.float().cuda()    # [batch, sample_num, 3]
        point_input = sample_points.float().cuda() # [batch, sample_num, 3]
        coord_input = coord_imgs.float().cuda()
        ...
        loss2 = mse_loss(pred_color_mean, ori_color_mean)
        loss3 = mse_loss(pred_color_var, ori_color_var)
        # print("Color Emb", color_embeddings.min().item(), color_embeddings.max().item())
        # print("Shape Emb", shape_conditions.min().item(), shape_conditions.max().item())
        # print("Feature Emb", feature_embeddings.min().item(), feature_embeddings.max().item())
        # print("Pred Result", pred_result.min().item(), pred_result.max().item())
        # print("Pred features", pred_features.min().item(), pred_features.max().item())
        # print("Pred Imgs", pred_imgs.min().item(), pred_imgs.max().item())
